...
```mermaid
flowchart LR
    UI["React + Vite<br/>Chat interface"] -->|HTTP / JSON| API["FastAPI API<br/>validation · PII scan · rate limits"]
    API --> Agent["ReAct agent<br/>guardrail → planner → executor"]
    Agent <-->|tool calls| Groq["Groq LLM"]
    Agent --> Tools["Typed tools<br/>+ repositories"]
    Agent <-->|session context| DB[("SQLite<br/>data + chat sessions")]
    Tools <--> DB
    Excel["Excel workbooks"] -->|seed| DB

    classDef ui fill:#0f172a,stroke:#38bdf8,color:#e0f2fe,stroke-width:2px;
    classDef backend fill:#3b0764,stroke:#c084fc,color:#f3e8ff,stroke-width:2px;
