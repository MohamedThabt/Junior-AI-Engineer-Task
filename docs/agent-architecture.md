...
**Security/PII utility (`app/utilities/security.py`):**
...
### 2.1 Planner (`app/agent/planner.py`)
...
### 2.2 Executor / Dispatcher (`app/agent/executor.py`)
...
### 2.3 Loop Controller (`app/agent/loop_controller.py`)
...
- `max_steps` is read from the environment at startup (`MAX_STEPS` in `.env`,
  default `5`) via `config/settings.py` — never hardcoded in the loop
  controller itself.
...
### 2.4 Finalize node (`app/agent/finalize.py`)
...
### 2.5 LLM Call Wrapper (`app/agent/llm_client.py`)
...
### 2.7 Session Controller (`app/controllers/session_controller.py`)
...
- Exposed via routes such as `GET /sessions`, `GET /sessions/{id}`,
  `DELETE /sessions/{id}` in `app/api/routes.py`.
...
(`ListingRepository`, `CampaignRepository`) for the actual SQL — the tool
...
storage engine later only touches `app/repositories/`, not `app/tools/`.
...
├── app/
│   ├── api/
│   │   └── routes.py            # FastAPI routes: /agent/chat, /sessions, /sessions/{id}
│   ├── controllers/
...
│   │   ├── llm_client.py        # LLM HTTP wrapper: retry, logging, cost calc
│   │   └── prompts.py           # system prompt template (section 4)
│   ├── tools/
│   │   ├── registry.py          # {tool_name: callable} + JSON schemas for function-calling
│   │   ├── listings_tools.py    # query/insert/update/delete for real_estate_listings (calls ListingRepository)
...
`app/tools/registry.py`. Every data tool below calls its matching repository
method for the actual SQL — no tool file contains raw SQL itself (see 7.1).

| # | Tool name | Verb | Table | Repository method called | Key args |
|---|---|---|---|---|---|
| 1 | `query_listings` | read | `real_estate_listings` | `ListingRepository.query()` | filters (city, state, status, price range, beds/baths range), `limit` (capped server-side) |
| 2 | `insert_listing` | create | `real_estate_listings` | `ListingRepository.insert()` | all listing fields |
| 3 | `update_listing` | modify | `real_estate_listings` | `ListingRepository.update()` | `listing_id` (required, exact), fields to change |
| 4 | `delete_listing` | delete | `real_estate_listings` | `ListingRepository.delete()` | `listing_id` (required, exact) |
...
- **One responsibility per tool** — one tool = one verb on one table (e.g.
  `update_listing` only updates `real_estate_listings`; it does not also
  handle campaigns or perform reads).
- **Deterministic** — given identical input *and identical database state*,
  output is identical. (The caveat matters: a `query_listings` call is
...
2. **Delegate the actual DB operation to the matching repository method** —
   `ListingRepository` / `CampaignRepository`. The tool file itself contains
...
4. On final failure (after 2 attempts), return a structured error result
   ...
   `"update_listing failed after 2 attempts: listing_id 'LST-9999' not
...
```json
{
  "success": false,
  "data": null,
  "error": "update_listing failed after 2 attempts: listing_id 'LST-9999' not found",
  "tool": "update_listing",
  "attempts": 2
}
```
...
1. **Machine-readable tool schemas.** The rules describe tool *behavior* but
   ...
   know how to call a tool correctly. Put these in `tools/registry.py`.
2. **Row limits on read tools.** `query_listings` / `query_campaigns` must
...
3. **Write-scope safety.** `update_listing`/`delete_listing` (and the
