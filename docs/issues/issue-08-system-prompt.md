...
### `build_system_prompt(max_steps: int) -> str`
Returns the full system prompt from §4 with `{max_steps}` interpolated at runtime, so the LLM's stated budget always matches the enforced limit.
...
- The **5 rules** exactly as written in §4:
  1. Max `{max_steps}` steps; don't look up what's already in the prompt.
...
## Notes
- Keep the prompt as a module-level template string; only `{max_steps}` is interpolated.
